...
def storeToFile(num_frames):
    ...
    with open(EMOTION_CSV_FILE, 'w', newline='') as outcsv:
        writer = csv.DictWriter(outcsv, fieldnames=["frame_no", "box", "emotion"])
        writer.writeheader()
        ...
        while iterations >= 0:
            for i in range(min(iterations, MAX_FRAMES_PER_MIN)):
                ...
                for e, e_value in response[0]["faceAttributes"]["emotion"].items():
                    if max_emotion_val < e_value:
                        emotion = e
                        max_emotion_val = e_value

                writer.writerow({"frame_no": cur_iteration*20 + i, "box": response[0]["faceRectangle"], 'emotion': emotion})
